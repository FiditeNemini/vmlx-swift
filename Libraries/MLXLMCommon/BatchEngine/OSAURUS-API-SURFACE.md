...
| `public struct MTPBundleStatusSnapshot: Codable, Sendable, Equatable` | Bundle-local native-MTP status JSON, including `has_usable_native_mtp_tuning`, `can_auto_launch`, `measured_family_auto_depth`, `requires_native_mtp_tuning_before_auto_launch`, `bundle_has_vision`, and nested `vmlx_mtp_tuning.json` metadata. The nested snapshot keeps the original `blocked` bit and separately reports `legacy_block_superseded` when the exact pre-PR-365 Qwen3.8 4M measurement is recognized. |
